...
export const useUserStore = create<State>((set, get) => ({
  ...
  onlineUpdate(data?: Record<string, People>) {
    if (data) {
      const newOnlinePeople = { ...get().onlinePeople };
      for (const address in data) {
        const people = data[address];
        if (isAddress(address) && typeof people.value === "string") {
          const timeDiff = dayjs().diff(dayjs(people.timestamp), "minute");
          // 如果超过5分钟，从列表中移除
          if (timeDiff > 5) {
            delete newOnlinePeople[address];
            continue;
          }
          // 5分钟内的用户，更新或添加
          if (get().onlinePeople[address]) {
            // 已存在的用户，直接更新为最新数据
            newOnlinePeople[address] = people;
          } else {
            // 新用户，直接添加
            newOnlinePeople[address] = people;
          }
      ...
      set({ onlinePeople: newOnlinePeople });
    }
  },
}));
